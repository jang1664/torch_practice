...
torch::Tensor add_cuda_forward(torch::Tensor x, torch::Tensor y) {
  ...
  float *x_gpu, *y_gpu, *out_gpu;
  cudaMalloc(&x_gpu, n * sizeof(float));
  cudaMalloc(&y_gpu, n * sizeof(float));
  cudaMalloc(&out_gpu, n * sizeof(float));

  cudaMemcpy(x_gpu, x.data_ptr<float>(), n * sizeof(float), cudaMemcpyHostToDevice);
  cudaMemcpy(y_gpu, y.data_ptr<float>(), n * sizeof(float), cudaMemcpyHostToDevice);
  add_cuda_forward_kernel<float><<<blocks, threads>>>(x_gpu, y_gpu, out_gpu, n);
  cudaMemcpy(out.data_ptr<float>(), out_gpu, n * sizeof(float), cudaMemcpyDeviceToHost);
